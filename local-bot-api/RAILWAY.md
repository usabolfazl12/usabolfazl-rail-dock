...
> ✅ چون از ایمیج از پیش کامپایل‌شده استفاده می‌کنیم، build فقط چند ثانیه طول می‌کشد (نه ۲۰+ دقیقه).

4. **Volume برای سرور:** Settings → Volumes → Add → mount path: `/var/lib/telegram-bot-api`
   (فایل‌های دانلودی/آپلودی اینجا ذخیره می‌شوند)
